...
class SourceAnalysisAttempt(BaseModel):
    ...
    # UMA vez a partir do CompletionRequest real de Source Analysis --
    # nunca faz o texto de fonte/análise mais autoritativo, é só
    # provenance do REQUEST. `None` é EXCLUSIVAMENTE o valor de uma
    # tentativa persistida ANTES deste slice existir.
    ...
    def _parse_status_and_error_message_are_consistent(self) -> SourceAnalysisAttempt:
        ...
        if self.parse_status in ("malformed", "interpretation_failed") and not (
            self.parse_error_message
        ):
            raise ValueError(
                "parse_status em {'malformed','interpretation_failed'} exige "
